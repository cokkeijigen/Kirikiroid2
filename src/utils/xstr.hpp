...
namespace xstr
{
	...
	template<class char_type>
	inline auto view<char_type>::trim() noexcept -> xstr::view<char_type>&
	{
		if (this->empty())
		{
			return *this;
		...
		if (end < begin)
		{
            this->view_t::operator=({});
		}
		else 
		{
            this->view_t::operator=(view_t{ begin, end + 1 });
		}
		return *this;
	}
	...
	{
        std::initializer_list<std::basic_string_view<char_t>> strs{ std::basic_string_view<char_t>{args}... };
        std::basic_string<char_t> result{};
        std::ranges::copy(strs | std::views::join, std::back_inserter(result));
        return result;
	}
	...
	{
		return xstr::view<char_type>(string).trim();
	}
